...
static const unsigned char ocp_fw_activation_history_guid[16] = {
	0x6D, 0x79, 0x9a, 0x76, 0xb4, 0xda, 0xf6, 0xa3, 0xe2, 0x4d,
	0xb2, 0x8a, 0xac, 0xf3, 0x1c, 0xd1 };
...
static void ocp_fw_activation_history_print(
	const struct fw_activation_history *fw_history)
{
	...
	const struct fw_activation_history *fw_history)
{
	...
	json_object_add_value_uint(root, "valid entries",
		le32_to_cpu(fw_history->valid_entries));
	...
	for (int index = 0; index < fw_history->valid_entries; index++) {
		...
		struct json_object *firmware_obj =
			json_object_new_string_len(entry->previous_fw, sizeof(entry->previous_fw));

		json_object_add_value_object(entry_obj, "previous firmware", firmware_obj);

		firmware_obj =
			json_object_new_string_len(entry->new_fw, sizeof(entry->new_fw));

		json_object_add_value_object(entry_obj, "new firmware", firmware_obj);
	...
	struct command *cmd, struct plugin *plugin)
{
	...
	const char *description =
		"Retrieves and parses OCP firmware activation history log.";
	...
	if (!err && !memcmp(fw_history.log_page_guid,
		ocp_fw_activation_history_guid,
		sizeof(ocp_fw_activation_history_guid)) == 0) {
		fprintf(stderr,
